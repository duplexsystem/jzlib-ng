...
public class FastDeflater {
     ...
     *
     * @see FastDeflater#deflate(byte[], int, int, int)
     * @since 1.7
     */
        ...
        synchronized (zsRef) {
            ...
            if (input == null) {
                ...
                if (input.isDirect()) {
                    ...
                    try {
                        if (output.isDirect()) {
                            long outputAddress = outputPos + UnsafeUtils.getFieldLong(output, "address");
                            try {
    ...
    static class DeflaterZStreamRef implements Runnable {
        ...
        private DeflaterZStreamRef(FastDeflater owner, long addr) {
